...
def generate_conversation_threadsafe(persona, index, output_path, question, method, args):
    ...
    while True:
        try:
            ...
            model = load_models(args, system_prompt = system_prompt, max_token=50)
            model.init_history()
            ...
            q1 = f"""
                 ...
                 Answer with the number of the option you choose only. 
                 Answer:
                 """
...
def run():
    ...
    parser.add_argument('--setting', type=str, default="small_town")
    ...
    for row in chars.itertuples():
        ...
        convos = pd.read_csv(f"data/moral_questions.csv")
        convos = convos["rot"].unique().tolist()    

        for question_idx, question in enumerate(convos):        

            persona_path = os.path.join(base_input_dir, f"{setting}.json")
            output_dir = os.path.join(base_output_dir, setting, f"answers_moral_{question_idx}")
